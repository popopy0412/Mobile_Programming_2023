...
import com.example.lec05.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    fun init2(){
        binding.button.setOnClickListener{
            val bmi = binding.weight.text.toString().toDouble() / (binding.height.text.toString().toDouble() / 100).pow(2)
    ...
    fun init(){
        ...
        button.setOnClickListener {
            val bmi = weight.text.toString().toDouble() / (height.text.toString().toDouble() / 100).pow(2)
            var resultString: String
            when{
                bmi >= 35 ->{
                    resultString = "고도 비만"
